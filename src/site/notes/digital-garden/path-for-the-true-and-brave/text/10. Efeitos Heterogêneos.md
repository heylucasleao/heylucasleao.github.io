...
O objetivo do CATE é fundamentar a decisão nas características específicas ($X$) de cada unidade. Mudamos a pergunta de _"O tratamento funciona?"_ para "Para quem o tratamento funciona melhor?".

Matematicamente:

$$\tau(x) = \mathbb{E}[Y_i(1) - Y_i(0) \mid X_i=x]$$

Onde buscamos a diferença esperada entre os potenciais outcomes ($Y_1$ e $Y_0$), condicionada às características $X$.
...
Isso prova matematicamente que o efeito não é mais constante ($\beta_1$): ele agora é uma função linear das características do indivíduo ($\beta_1 + \beta_3 X_i$).
...
Diferente de Machine Learning tradicional (onde temos o $Y$ real para calcular RMSE ou Acurácia), na inferência causal nunca observamos o efeito real individual (não vemos $Y_1$ e $Y_0$ simultaneamente).
...
Utilizamos métricas de "Uplift" para validar essa ordenação. Recomendo a leitura do capítulo 6 do livro [Causal Inference in Python](https://www.amazon.com.br/Causal-Inference-Python-Applying-Industry/dp/1098140257).

>[!note] Exemplos visuais
 >Recomendo que observe os [exemplos](https://github.com/matheusfacure/causal-inference-in-python-code/blob/main/causal-inference-in-python/06-Effect-Heterogeneity.ipynb) do livro junto com as anotações.
