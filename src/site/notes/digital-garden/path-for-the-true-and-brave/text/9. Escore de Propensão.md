...
Onde $T$ é a variável de tratamento e $X$ covariáveis.

Ao condicionarmos nas covariáveis, o escore de propensão controla as variáveis de confusão, ajudando a alcançarmos a independência condicional — o tratamento T torna-se independente do resultado potencial, desde que se condicione no escore de propensão, $T \perp Y^t | e(X)$.
...
O peso para cada unidade $\mathbf{W}_i$ é calculado da seguinte forma, onde $\mathbf{T}_i$ é o tratamento real recebido:
...
- $\mathbf{T}_i$ é a variável de tratamento.
...
O termo $\frac{\mathbf{T}_i - \hat{e}(\mathbf{X}_i)}{\hat{e}(\mathbf{X}_i)}$ tenderá a zero na média, e o segundo termo todo se anulará.
