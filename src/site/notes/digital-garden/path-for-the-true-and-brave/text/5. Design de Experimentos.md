...
> [!tip] Premissas Importantes
	> Mesmo não sendo uma "bala de prata", o RCT é o padrão-ouro. Contudo, existem premissas fundamentais (comuns em estudos observacionais) que também **precisam** ser respeitadas nele:
...
Em um RCT, este mecanismo é controlado e conhecido pelo pesquisador (ex: lançar uma moeda, sorteio por computador). Em estudos observacionais, este mecanismo é desconhecido e precisa ser estimado.

Documentar e definir explicitamente o mecanismo de atribuição é vital, pois ele:

1. Define o Escore de Propensão: O mecanismo de atribuição é a base matemática para o cálculo do _score_ de propensão, $e(x) = P(W=1|X)$. Se o design for aleatorizado, temos uma aproximação boa desse valor; se for observacional, precisamos estimá-lo para rebalancear os grupos ou utilizarmos pseudo-populações. [[digital-garden/path-for-the-true-and-brave/text/9. Escore de Propensão\|9. Escore de Propensão]]
    
2. Determina o Cálculo da Variância: A forma como a randomização foi feita muda a forma como calculamos o erro padrão.
    - _Randomização Simples:_ Assume independência entre unidades.
    - _Randomização em Clusters:_ Exige ajuste de erros padrão clusterizados, pois unidades no mesmo cluster não são independentes.
        
3. Ignorabilidade: Confirma se a atribuição do tratamento foi feita independentemente dos resultados potenciais ($Y(0), Y(1)$).