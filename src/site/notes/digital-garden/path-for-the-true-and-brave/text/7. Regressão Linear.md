...
>_[Linear regression: our go-to tool](https://www.everydaycausal.com/data-models-causality.html)_

O modelo de regressão linear é a ferramenta mais tradicional para estimarmos impactos de tratamento. Estruturalmente, ela é uma comparação entre médias, estimando como o valor médio da variável dependente muda conforme alteramos os valores das variáveis independentes. Essa característica faz com que consigamo estimar efeitos médios por ela, assim como controlar variáveis condicionais e relações complexas.

> [!tip] Análise entre médias de diferentes grupos
> É importante notar que, em cenários com diferentes grupos, se formos analisar a mão, podemos analisar o tratamento sobre efeito ponderado, isto é, avaliando o efeito pelo tamanho de amostra.
> Entretanto, o mesmo não ocorre internamente na regressão linear. No caso, os grupos serão ponderados pela variância de valores entre eles.
> 
> A intuição por trás disso é que o modelo busca extrair o máximo de informação possível: grupos onde a variável apresenta maior dispersão (em dummies, quando a proporção de tratados e controles é mais equilibrada) oferecem mais "contraste" para o estimador, recebendo, portanto, maior peso na composição do coeficiente final. Em casos de tratamentos categóricos com efeitos heterogêneos, isso significa que o $\beta$ estimado será mais influenciado pelos grupos onde o tratamento varia mais, o que pode gerar uma divergência entre o coeficiente da regressão e o ATE puramente populacional.

Utilizando o método dos Mínimos Quadrados Ordinários (MQO), estimamos parâmetros que, sob as suposições corretas de identificação, possuem uma interpretação causal direta. Considerando uma relação linear simples $Y \sim X + \varepsilon$, estimamos o efeito de $X$ sobre $Y$ através dos coeficientes $\hat{\beta}_0$ e $\hat{\beta}_1$.
...
>[!Abstract] Leitura Recomendada
> _[Causal Inference in Python: Applying Causal Inference](https://www.amazon.com/Causal-Inference-Python-Applying-Industry/dp/1098140257)_ — Part II, Cap. Frisch-Waugh-Lovell Theorem and Orthogonalization, pág. 106
...
Para ver exemplos via código, [é só acessar aqui.](https://github.com/matheusfacure/causal-inference-in-python-code)
...
> [!tip] Lembrete
> O $ATE$ é calculado como a diferença média entre o que toda a população teria se fosse tratada $\hat{E}[Y_1 | X_i]$ e o que toda a população teria se não fosse tratada $\hat{E}[Y_0 | X_i]$
...
> [!tip] Lembrete
> O $ATT$ é a diferença média entre o resultado **observado** para o grupo tratado $Y_i$ e o seu resultado contrafactual imputado $\hat{E}[Y_0 | X_i]$
